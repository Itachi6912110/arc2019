function [positions, time] = tracker_mod(num_frame, video_path, img_files, pos, target_sz, ...
	...
	for frame = 1:numel(img_files)
		...
		if frame >= num_frame
			...
			if frame > num_frame
				...
				zf = fft2(get_features_mod(patch, cos_window));
			...
			patch = get_subwindow(im, pos, window_sz);
			xf = fft2(get_features_mod(patch, cos_window));	

			%Kernel Ridge Regression, calculate alphas (in Fourier domain)
			kf = gaussian_correlation(xf, xf, kernel.sigma);
			alphaf = yf ./ (kf + lambda);   %equation for fast training	

			if frame == num_frame  %first frame, train with a single image
				model_alphaf = alphaf;
				model_xf = xf;
			else
